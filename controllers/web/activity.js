...
async function deleteData(req, res) {
    ...
    try {
        ...
        let getData = await ActivitySchema.findOne({ "_id": ObjectID(_id)});
        ...
        const dataRemoved = await ActivitySchema.deleteOne({ "_id": ObjectID(_id)});
...
async function getDetails(req, res) {
    try {
        ...
        const recordDetail = await ActivitySchema.findOne({_id:ObjectID(_id)});
    ...
    try {
        ...
        if (page > 0) {
           ...
           res.status(200).json(await Response.pagination(allRecords, responseMessage(reqObj.langCode,'SUCCESS'),pageData,req));
            ...
            let allRecords = await ActivitySchema.find({}).lean();
